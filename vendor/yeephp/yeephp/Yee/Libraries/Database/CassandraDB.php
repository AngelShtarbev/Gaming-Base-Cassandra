...
class CassandraDB
{
    ...
    /**
     * Pass in a raw query and execute it. If the query has to be prepared, table name and data must be included
     * 
     * @param string $query             Contains a user-provided query.
     * @param string $tableName    Name of the table where the query will execute the prepared statement
     * @param array $data                Array with the data which will be binded to the prepared statement
     * 
     * @return array | boolean          Contains the returned rows from the query OR the state of the executed query.
     */
    public function rawQuery( $query, $tableName = null, $data = null )
    {
        if ( !$this->isConnected ) {
            $this->connect();
        }
        ...
        //Get table info so that the variables can be convertd to cassandra object
        if ( $data != null ) {
            $this->_getTableInfo( $this->_keyspace->table( $tableName ) );
        }

        $stmt = $this->_buildQueryStatement( $data == null );

        $arguments = $this->_buildExecutionOptions( $data );
        ...
        if ( $result->count() > 0 ) {
            $result = $this->_extractRows( $result );
            $this->reset();
            return $result;
        }
    ...
    protected function _convertToCassandraObject( $paramValue, $paramKey )
    {
        ...
        switch ( $this->_table[$paramKey] ) {
            case 'timestamp':
                return new Cassandra\Timestamp( is_int( $paramValue ) ? $paramValue : strtotime( $paramValue )  );
            case 'bigint':
